...
from open_webui.utils.auth import get_admin_or_security_curator_user, get_admin_user, get_verified_user
    ...
    try:
        ...
        if group:
            set_audit_event(
                ...
                changes={'created': group.model_dump()},
            )
    ...
    try:
        ...
        if group:
            set_audit_event(
                ...
                actor=user.model_dump(include={'id', 'name', 'email', 'role'}),
                target={'type': 'group', 'id': group.id, 'name': group.name},
                changes={'updated': form_data.model_dump()},
            )
            return GroupResponse(
                **group.model_dump(),
                member_count=await Groups.get_group_member_count_by_id(group.id, db=db),
    ...
    try:
        ...
        if group:
            set_audit_event(
                ...
                outcome='success',
                actor=user.model_dump(include={'id', 'name', 'email', 'role'}),
                target={'type': 'group', 'id': id, 'name': group.name},
                changes={'added_user_ids': form_data.user_ids},
            )
            return GroupResponse(
                **group.model_dump(),
                member_count=await Groups.get_group_member_count_by_id(group.id, db=db),
    ...
    try:
        ...
        if group:
            set_audit_event(
                ...
                actor=user.model_dump(include={'id', 'name', 'email', 'role'}),
                target={'type': 'group', 'id': id, 'name': group.name},
                changes={'removed_user_ids': form_data.user_ids},
            )
            return GroupResponse(
                **group.model_dump(),
                member_count=await Groups.get_group_member_count_by_id(group.id, db=db),
