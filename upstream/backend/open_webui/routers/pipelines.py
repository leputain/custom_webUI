...
from open_webui.utils.auth import get_admin_or_security_curator_user, get_admin_user
...
@router.get('/list')
async def get_pipelines_list(request: Request, user=Depends(get_admin_or_security_curator_user)):
...
@router.get('/')
async def get_pipelines(request: Request, urlIdx: Optional[int] = None, user=Depends(get_admin_or_security_curator_user)):
    response = None
    try:
        url = request.app.state.config.OPENAI_API_BASE_URLS[urlIdx]
        key = request.app.state.config.OPENAI_API_KEYS[urlIdx]
