...
from open_webui.utils.auth import (
    ADMIN_ROLE,
    get_admin_or_security_curator_user,
    get_admin_user,
    get_password_hash,
    get_verified_user,
    PENDING_ROLE,
    SECURITY_CURATOR_ROLE,
    USER_ROLE,
    validate_password,
)
from open_webui.utils.audit import changed_fields, set_audit_event
...
async def get_default_user_permissions(request: Request, user=Depends(get_admin_or_security_curator_user)):
    return {
...
async def update_default_user_permissions(request: Request, form_data: UserPermissions, user=Depends(get_admin_user)):
    before = request.app.state.config.USER_PERMISSIONS
    ...
    set_audit_event(
        ...
        changes=changed_fields(before, request.app.state.config.USER_PERMISSIONS),
    )
...
async def update_user_by_id(
    ...
    if user:
        if form_data.role is not None and form_data.role not in {
            ADMIN_ROLE,
            USER_ROLE,
            PENDING_ROLE,
            SECURITY_CURATOR_ROLE,
        }:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGES.DEFAULT('Invalid role'))

        before_user = user.model_dump(include={'id', 'name', 'email', 'role', 'profile_image_url'})
        ...
        if updated_user:
            ...
            after_user = updated_user.model_dump(include={'id', 'name', 'email', 'role', 'profile_image_url'})
            changes = changed_fields(before_user, after_user)
            if form_data.password:
                changes['password'] = {'changed': True}
            if changes.get('role', {}).get('to') == SECURITY_CURATOR_ROLE:
                event_type = 'user.security_curator.assigned'
            elif changes.get('role', {}).get('from') == SECURITY_CURATOR_ROLE:
                event_type = 'user.security_curator.removed'
            ...
            set_audit_event(
                ...
                event_type,
                outcome='success',
                actor=session_user.model_dump(include={'id', 'name', 'email', 'role'}),
                target={'id': updated_user.id, 'email': updated_user.email},
                changes=changes,
            )
            return updated_user
