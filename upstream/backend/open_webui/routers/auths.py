    ...
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        if user:
            ...
            if WEBUI_AUTH_TRUSTED_ROLE_HEADER:
                ...
                if trusted_role in {ADMIN_ROLE, USER_ROLE, PENDING_ROLE, SECURITY_CURATOR_ROLE}:
...
async def add_user(
    ...
    admin_user = user
    if form_data.role not in {ADMIN_ROLE, USER_ROLE, PENDING_ROLE, SECURITY_CURATOR_ROLE}:
    ...
    try:
        ...
        if user:
            ...
            set_audit_event(
                ...
                'user.created',
                outcome='success',
                actor=admin_user.model_dump(include={'id', 'name', 'email', 'role'}),
                target={'id': user.id, 'email': user.email},
                changes={
                    'created': {
                        'email': user.email,
...
async def get_admin_config(request: Request, user=Depends(get_admin_or_security_curator_user)):
    return {
...
async def update_admin_config(request: Request, form_data: AdminConfig, user=Depends(get_admin_user)):
    ...
    changes = changed_fields(security_config_before, security_config_after)
    if changes:
        set_audit_event(
            request,
            'security.settings.updated',
            outcome='success',
            target={'type': 'security_settings'},
            changes=changes,
        )
...
async def get_ldap_server(request: Request, user=Depends(get_admin_or_security_curator_user)):
    return {
...
async def get_ldap_config(request: Request, user=Depends(get_admin_or_security_curator_user)):
...
async def generate_api_key(
    ...
    if user.role == SECURITY_CURATOR_ROLE:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
