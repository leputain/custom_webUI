...
SECURITY_CURATOR_ROLE = 'security_curator'
VERIFIED_ROLES = {USER_ROLE, ADMIN_ROLE, SECURITY_CURATOR_ROLE}
ADMIN_READ_ROLES = {ADMIN_ROLE, SECURITY_CURATOR_ROLE}
...
def get_verified_user(user=Depends(get_current_user)):
    if user.role not in VERIFIED_ROLES:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_MESSAGES.ACCESS_PROHIBITED,
        )
...
def get_admin_or_security_curator_user(user=Depends(get_current_user)):
    if user.role not in ADMIN_READ_ROLES:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=ERROR_MESSAGES.ACCESS_PROHIBITED,
        )
...
def get_security_read_user(user=Depends(get_current_user)):
    return get_admin_or_security_curator_user(user)


def require_real_admin(user=Depends(get_current_user)):
