...
app.include_router(security.router, prefix='/api/v1/admin/security', tags=['admin-security'])
...
async def get_base_models(request: Request, user=Depends(get_admin_or_security_curator_user)):
...
@app.get('/api/tasks')
async def list_tasks_endpoint(request: Request, user=Depends(get_admin_or_security_curator_user)):
...
@app.get('/api/webhook')
async def get_webhook_url(user=Depends(get_admin_or_security_curator_user)):
    return {
