<script lang="ts">
	...
	import { canAccessAdminPanel, isActiveAppRole } from '$lib/utils/rbac';
	...
	onMount(async () => {
		...
		if (canAccessAdminPanel($user) && ($settings?.showChangelog ?? true)) {
		...
		if (canAccessAdminPanel($user) && $config?.features?.enable_version_update_check) {
