<script lang="ts">
	...
	import { isReadOnlyAdmin, roleLabel } from '$lib/utils/rbac';
					...
					<tr class="bg-white dark:bg-gray-900 dark:border-gray-850 text-xs">
						<td class="px-3 py-1 min-w-[7rem] w-28">
							<button
								...
								aria-label={$i18n.t(readOnlyAdmin ? 'Role' : 'Change User Role')}
							...
							>
								<Badge
									type={user.role === 'admin'
										...
										: user.role === 'user'
											? 'success'
											: user.role === 'security_curator'
												? 'warning'
												: 'muted'}
									content={roleLabel(user.role)}
								/>
							</button>
						</td>
						...
						<td class="px-3 py-1 text-right">
							<div class="flex justify-end w-full">
								...
								{#if !readOnlyAdmin && user.role !== 'admin'}
